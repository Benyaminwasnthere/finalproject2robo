...
if __name__ == "__main__":
  ...
  arms_no = arms.shape[0]

  arms_list = [arm(arms[i][0],arms[i][1]) for i in range(arms_no)]

  target_arm = arm(target_x,target_y)
  target_arm.do_patch()

  #start_time = time.time()
  distance_list = [(i,euclidean_distance(target_arm.node, arm.node)) for i, arm in enumerate(arms_list)]
  distance_list = sort_tuple(distance_list)

  for i in range(k if k<len(distance_list) else len(distance_list)):
    temp_arm =arms_list[distance_list[i][0]]
    if i==0:
